...
(function ($) {
    ...
    $.fn.AvRecorderMoz = function (id, conf) {
        ...
        if (settings.constraints.video == true) {
            setStatus('Click \'Turn on micro\' to enable your mic and camera.');
        } else {
            setStatus('Click \'Turn on micro\' to enable your mic.');
        }
        ...
        function createVolumeMeter() {
            ...
            meterProcessor.onaudioprocess = function () {
                ...
                if (canvasContext != null) {
                    canvasContext.arc(0, 0, $meter[0].width, $meter[0].height);
...
//          recordURL = URL.createObjectURL(localStream);
                    format = constraints.video ? 'webm' : 'mp3';
                    mimetype = constraints.video ? 'video/webm' : 'mpeg/audio';
...
//          recorder = new MediaRecorder(localStream, options);
                    ...
                    setStatus('Press button to start recording.');
                    if (callback != null) {
                        callback();
                    }
                })
...
//        setStatus('Record with audio or video?');
        ...
        function sendBlob(blob) {
            ...
            if (currentSeconds < 15) {
                alertAppend('Audio cannot be shorter than 15 sec', 'danger', 'oh nooo!');
                refreshRecord();
                $('.loading').fadeOut();
            ...
            request.onload = function (evt) {
                ...
                // createNewRowRecord('/storage/' + jsonObj.url, jsonObj.audio_id);
                ...
                $(".loader").hide();
                refreshRecord(jsonObj.time);
                if (jsonObj.time < 0) {
                    $('.appAppearAprove').html(jsonObj.approveBlock);
                } else {
                    $('.appAppearAprove').html('' +
                        ...
                        '\n' +
                        '                                            <p class="tx-danger">You cannot send task before you do not finish it.</p>\n' +
                        '                                        </div>\n' +
                        '                                    </div>\n');
                }
...
//            $video.show();
        ...
        function initializeEvents() {
            ...
            $element.bind('recordStart', function (event, data) {
                ...
                var timeLimitFormatted = 0;
                timeLimitFormatted = millisecondsToTime(new Date(parseInt(settings.time_limit, 10) * 1000));
